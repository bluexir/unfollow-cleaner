  ...
  if (error) {
    return (
      <div className="bg-red-500/10 border border-red-500/50 p-6 rounded-lg text-center backdrop-blur-sm">
        ...
        <button onClick={fetchNonFollowers} className="mt-4 px-4 py-2 bg-red-600/20 hover:bg-red-600/40 text-red-200 rounded border border-red-500/30 transition-all">
          Tekrar Dene
        </button>
      </div>
    );
    ...
    <div className="space-y-8 relative min-h-screen pb-24 animate-fade-up">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 border-b border-gray-800 pb-4">
          ...
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-2 cursor-pointer group">
              ...
              <input type="checkbox" className="hidden" onChange={toggleSelectAll} checked={selectedUsers.size === nonFollowers.length} />
              <span className="text-sm font-medium text-gray-400 group-hover:text-white transition-colors">Tümünü Seç</span>
            </label>
            {selectedUsers.size > 0 && (
              <button onClick={handleBulkUnfollowClick} disabled={isUnfollowing} className="bg-red-600 hover:bg-red-500 disabled:bg-gray-800 disabled:text-gray-500 text-white font-bold py-2 px-6 rounded-lg shadow-[0_0_20px_rgba(220,38,38,0.3)] transition-all flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {nonFollowers.map((user) => (
          <div key={user.fid} className={`group relative p-4 rounded-xl border transition-all duration-200 flex items-center gap-4 ${selectedUsers.has(user.fid) ? 'bg-red-900/10 border-red-500/50' : 'bg-black/40 border-gray-800 hover:border-gray-600'}`}>
      ...
      {sessionCount > 0 && (
        <div className="fixed bottom-6 right-6 z-50 animate-bounce-in">
          <div className="bg-black border border-gray-700 shadow-2xl rounded-full pl-6 pr-2 py-2 flex items-center gap-6">
            ...
            <button onClick={() => setShowSharePopup(true)} className="bg-white text-black hover:bg-gray-200 font-bold py-2 px-6 rounded-full text-sm flex items-center gap-2">
              <span>PAYLAŞ</span>
